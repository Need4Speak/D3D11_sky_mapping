...
void CleanUp()
{
	...
	carVertBuffer->Release();
	carIndexBuffer->Release();
	wheelVertBuffer->Release();
	wheelIndexBuffer->Release();
	VS->Release();
	PS->Release();
	VS_Buffer->Release();
	PS_Buffer->Release();
...
void InitBoxBuffer() {
	...
	iinitData.pSysMem = indices;
	d3d11Device->CreateBuffer(&indexBufferDesc, &iinitData, &carIndexBuffer);
	...
	UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	for (UINT i = 0; i <= sliceCount; ++i)
	{
		...
		meshData.Vertices.push_back(Vertex(x, y, z, u, v, 0.0f, 1.0f, 0.0f));
	}

	// Cap center vertex.
	meshData.Vertices.push_back(Vertex(0.0f, y, 0.0f, 0.5f, 0.5f, 0.0f, 1.0f, 0.0f));
...
bool InitScene()
{
	...
	//Camera information
	camPosition = XMVectorSet( 0.0f, 40.0f, -80.0f, 0.0f );
	...
	cmdesc.FillMode = D3D11_FILL_SOLID; //D3D11_FILL_WIREFRAME;D3D11_FILL_SOLID
	...
	hr = d3d11Device->CreateRasterizerState(&wfdesc, &CarcullMode);
